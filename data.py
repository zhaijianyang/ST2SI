...
class RecDataset(Dataset):
    def __init__(self, user2train, user2val, user2test, tokenized_items, args, mode, tokenizer=None):
        ...
        if self.mode == 'train':
            self.users = [user for user, seq in self.user2train.items() if len(seq) > 1]
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            items_seq_all = self.user2train[user]
            ...
            items_seq = self.user2train[user]
            label = self.user2val[user][0]
        else:
            items_seq = self.user2train[user] + self.user2val[user]
            label = self.user2test[user][0]
